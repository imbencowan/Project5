...
class Validate {
    ...
    public function getFields() {
	 ...
	 //validate whether two fields match
	 public function fieldsMatch($name1, $value1, $name2, $value2) {
			//get the field objects
		 $field1 = $this->fields->getField($name1);
		 $field2 = $this->fields->getField($name2);
		 
		 if ($value1 != $value2) {
			 $field2->setErrorMessage('Emails must match');
		 } else {
			 $field2->clearErrorMessage();
		 }
	 }
    ...
    public function text($name, $value,
	 ...
	 public function number($name, $value, $required = true, $min = 1, $max = 12) {
		$field = $this->fields->getField($name);
